...
# =============================================================================
# SECCIÓN: IMPLEMENTACIÓN Y COMPARACIÓN DE OTROS MODELOS
# =============================================================================

cat("\n=== IMPLEMENTANDO Y COMPARANDO OTROS MODELOS ===\n")

# Preparar datos para modelos
X_train_final <- as.data.frame(X_train)
y_train_final <- y_train

# Función para calcular métricas
calcular_metricas <- function(predicciones, real) {
  mse <- mean((real - predicciones)^2)
  rmse <- sqrt(mse)
  mae <- mean(abs(real - predicciones))
  mape <- mean(abs((real - predicciones) / real)) * 100
...
evaluar_modelo <- function(modelo, nombre_modelo, X_train, y_train) {
  cat("Entrenando", nombre_modelo, "...\n")
  ...
  tryCatch({
    ...
    # Calcular métricas
    metricas <- calcular_metricas(preds, y_train)
    ...
    cat("✅", nombre_modelo, "completado - RMSE:", round(metricas$RMSE, 2), "\n")
    return(resultados)
    
  }, error = function(e) {
    cat("❌ Error en", nombre_modelo, ":", e$message, "\n")
    return(NULL)
  })
}
...
# Preparar datos para LM (sin NAs)
train_lm <- bind_cols(price = y_train_final, X_train_final) %>%
  na.omit()

lm_model <- lm(price ~ ., data = train_lm)
resultados_lm <- evaluar_modelo(lm_model, "Regresión Lineal", 
                                train_lm %>% select(-price), 
                                train_lm$price)
...
X_matrix <- as.matrix(X_train_final)
y_vector <- y_train_final

# Remover NAs para glmnet
completos <- complete.cases(X_matrix) & !is.na(y_vector)
X_matrix_clean <- X_matrix[completos, ]
y_vector_clean <- y_vector[completos]

# Entrenar Elastic Net con validación cruzada
set.seed(123)
en_model <- cv.glmnet(X_matrix_clean, y_vector_clean, 
                      alpha = 0.5,  # Mix entre Lasso y Ridge
                      nfolds = 5)

# Predecir
preds_en <- predict(en_model, newx = X_matrix_clean, s = "lambda.min")[,1]

# Calcular métricas
metricas_en <- calcular_metricas(preds_en, y_vector_clean)

resultados_en <- list(
  modelo = en_model,
  predicciones = preds_en,
  metricas = metricas_en,
  nombre = "Elastic Net"
)

cat("✅ Elastic Net completado - RMSE:", round(metricas_en$RMSE, 2), "\n")

# =============================================================================
# 3. ÁRBOLES DE DECISIÓN (CART)
# =============================================================================
...
cart_model <- rpart(price ~ ., 
                    data = bind_cols(price = y_train_final, X_train_final),
                    method = "anova",
                    control = rpart.control(cp = 0.01, maxdepth = 10))

resultados_cart <- evaluar_modelo(cart_model, "Árbol de Decisión", 
...
# Entrenar Random Forest (versión rápida para comparación)
rf_model <- ranger(price ~ ., 
                   data = bind_cols(price = y_train_final, X_train_final) %>% na.omit(),
                   num.trees = 100,
                   mtry = floor(ncol(X_train_final) / 3),
                   min.node.size = 5,
                   seed = 123)

resultados_rf <- evaluar_modelo(rf_model, "Random Forest", 
                                X_train_final, y_train_final)
...
dtrain <- xgb.DMatrix(
  data = as.matrix(X_train_final[complete.cases(X_train_final), ]),
  label = y_train_final[complete.cases(X_train_final)]
)
...
# Predecir
preds_xgb <- predict(xgb_model, as.matrix(X_train_final[complete.cases(X_train_final), ]))

# Calcular métricas
metricas_xgb <- calcular_metricas(preds_xgb, y_train_final[complete.cases(X_train_final)])

resultados_xgb <- list(
  modelo = xgb_model,
  predicciones = preds_xgb,
  metricas = metricas_xgb,
  nombre = "XGBoost"
)

cat("✅ XGBoost completado - RMSE:", round(metricas_xgb$RMSE, 2), "\n")
...
X_scaled <- as.data.frame(lapply(X_train_final, scale_data))
y_scaled <- scale_data(y_train_final)

# Entrenar red neuronal
nn_model <- nnet(x = X_scaled[complete.cases(X_scaled), ], 
                 y = y_scaled[complete.cases(X_scaled)],
                 size = 10,
                 linout = TRUE,
                 maxit = 200,
                 trace = FALSE)

# Predecir y revertir escala
preds_nn_scaled <- predict(nn_model, X_scaled[complete.cases(X_scaled), ])
preds_nn <- preds_nn_scaled * sd(y_train_final, na.rm = TRUE) + mean(y_train_final, na.rm = TRUE)

# Calcular métricas
metricas_nn <- calcular_metricas(preds_nn, y_train_final[complete.cases(X_scaled)])

resultados_nn <- list(
  modelo = nn_model,
  predicciones = preds_nn,
  metricas = metricas_nn,
  nombre = "Red Neuronal"
)

cat("✅ Red Neuronal completada - RMSE:", round(metricas_nn$RMSE, 2), "\n")

# =============================================================================
# 7. COMPARACIÓN COMPLETA DE MODELOS
# =============================================================================
...
modelos_exitosos <- todos_resultados[!sapply(todos_resultados, is.null)]

# Crear tabla comparativa
tabla_comparativa <- map_dfr(modelos_exitosos, function(res) {
  data.frame(
    Modelo = res$nombre,
    RMSE = round(res$metricas$RMSE, 2),
    MAE = round(res$metricas$MAE, 2),
    MAPE = round(res$metricas$MAPE, 2),
    MSE = round(res$metricas$MSE, 2)
  )
})

# Ordenar por RMSE (mejor a peor)
tabla_comparativa <- tabla_comparativa %>%
  arrange(RMSE)

cat("\n📊 TABLA COMPARATIVA DE MODELOS (ORDENADO POR RMSE):\n")
print(tabla_comparativa)

# =============================================================================
# 8. ANÁLISIS VISUAL DE COMPARACIÓN
# =============================================================================
...
plot_rmse <- ggplot(tabla_comparativa, aes(x = reorder(Modelo, RMSE), y = RMSE)) +
  geom_col(fill = "steelblue", alpha = 0.8) +
  geom_text(aes(label = RMSE), vjust = -0.5, size = 3) +
...
# Gráfico de predicciones vs reales para los 3 mejores modelos
mejores_modelos <- head(tabla_comparativa$Modelo, 3)

for(modelo_nombre in mejores_modelos) {
  modelo_idx <- which(sapply(modelos_exitosos, function(x) x$nombre == modelo_nombre))
  resultados <- modelos_exitosos[[modelo_idx]]
  
  # Crear dataframe para el gráfico
  plot_data <- data.frame(
    Real = y_train_final[1:length(resultados$predicciones)],
    Predicho = resultados$predicciones
  )
  
  plot_pred <- ggplot(plot_data, aes(x = Real, y = Predicho)) +
    geom_point(alpha = 0.6, color = "blue") +
    geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
    labs(title = paste("Predicciones vs Reales -", modelo_nombre),
         x = "Precio Real",
         y = "Precio Predicho") +
    theme_minimal()
...
cat("\n--- ANÁLISIS DE RESIDUOS DE LOS MEJORES MODELOS ---\n")

for(modelo_nombre in mejores_modelos) {
  modelo_idx <- which(sapply(modelos_exitosos, function(x) x$nombre == modelo_nombre))
  resultados <- modelos_exitosos[[modelo_idx]]
  
  residuos <- y_train_final[1:length(resultados$predicciones)] - resultados$predicciones
  
  plot_resid <- ggplot(data.frame(Residuos = residuos), aes(x = Residuos)) +
    geom_histogram(fill = "coral", alpha = 0.7, bins = 30) +
    labs(title = paste("Distribución de Residuos -", modelo_nombre),
         x = "Residuos",
         y = "Frecuencia") +
    theme_minimal()
  ...
  cat("\n📈 Estadísticas de residuos para", modelo_nombre, ":\n")
  cat("   - Media de residuos:", round(mean(residuos), 2), "\n")
  cat("   - Desviación estándar:", round(sd(residuos), 2), "\n")
  cat("   - Residuos dentro de ±100M:", 
      round(mean(abs(residuos) < 100000000) * 100, 1), "%\n")
}
...
cat("🎯 MODELO CON MEJOR RENDIMIENTO:", tabla_comparativa$Modelo[1], "\n")
cat("   - RMSE:", tabla_comparativa$RMSE[1], "\n")
cat("   - MAE:", tabla_comparativa$MAE[1], "\n")
cat("   - MAPE:", tabla_comparativa$MAPE[1], "%\n")

cat("\n📈 COMPARACIÓN ENTRE FAMILIAS DE ALGORITMOS:\n")

# Agrupar por tipo de modelo
tabla_comparativa <- tabla_comparativa %>%
  mutate(Tipo = case_when(
    str_detect(Modelo, "Lineal") ~ "Lineales",
    str_detect(Modelo, "Elastic") ~ "Regularización",
    str_detect(Modelo, "Árbol") ~ "Árboles Simples",
    str_detect(Modelo, "Random") ~ "Ensambles",
    str_detect(Modelo, "XGBoost") ~ "Boosting",
    str_detect(Modelo, "Neuronal") ~ "Redes Neuronales",
    TRUE ~ "Otros"
  ))

resumen_familias <- tabla_comparativa %>%
  group_by(Tipo) %>%
  summarise(
    Mejor_RMSE = min(RMSE),
    Promedio_RMSE = mean(RMSE),
    N_Modelos = n()
  ) %>%
  arrange(Mejor_RMSE)
...
cat("\n💡 CONCLUSIONES:\n")
cat("1. Mejor familia:", resumen_familias$Tipo[1], "\n")
cat("2. Mejor modelo individual:", tabla_comparativa$Modelo[1], "\n")
cat("3. Diferencia entre mejor y peor modelo:", 
    round(max(tabla_comparativa$RMSE) - min(tabla_comparativa$RMSE), 2), "en RMSE\n")
cat("4. Número total de modelos evaluados:", nrow(tabla_comparativa), "\n")

# Guardar resultados comparativos
write_csv(tabla_comparativa, "comparacion_modelos.csv")
cat("\n💾 Resultados guardados en: comparacion_modelos.csv\n")

# =============================================================================
# 11. PREDICCIONES DE LOS OTROS MODELOS EN TEST
# =============================================================================

cat("\n=== GENERANDO PREDICCIONES DE OTROS MODELOS EN TEST ===\n")

# Función para predecir en test
predecir_test <- function(modelo, nombre_modelo, X_test) {
  cat("Generando predicciones para", nombre_modelo, "...\n")
  
  tryCatch({
    if(nombre_modelo == "Regresión Lineal") {
      preds <- predict(modelo, newdata = X_test)
    } else if(nombre_modelo == "Elastic Net") {
      X_test_matrix <- as.matrix(X_test)
      preds <- predict(modelo, newx = X_test_matrix, s = "lambda.min")[,1]
    } else if(nombre_modelo == "Árbol de Decisión") {
      preds <- predict(modelo, newdata = X_test)
    } else if(nombre_modelo == "Random Forest") {
      preds <- predict(modelo, data = X_test)$predictions
    } else if(nombre_modelo == "XGBoost") {
      X_test_matrix <- as.matrix(X_test)
      preds <- predict(modelo, X_test_matrix)
    } else if(nombre_modelo == "Red Neuronal") {
      X_test_scaled <- as.data.frame(lapply(X_test, scale_data))
      preds_scaled <- predict(modelo, X_test_scaled)
      preds <- preds_scaled * sd(y_train_final, na.rm = TRUE) + mean(y_train_final, na.rm = TRUE)
    } else {
      return(NULL)
    }
    
    # Redondear a 500,000 COP
    preds_redondeadas <- round(preds / 500000) * 500000
    
    return(preds_redondeadas)
    
  }, error = function(e) {
    cat("❌ Error en predicción de", nombre_modelo, ":", e$message, "\n")
    return(NULL)
  })
}

# Generar predicciones para los 3 mejores modelos
for(i in 1:min(3, nrow(tabla_comparativa))) {
  modelo_nombre <- tabla_comparativa$Modelo[i]
  modelo_idx <- which(sapply(modelos_exitosos, function(x) x$nombre == modelo_nombre))
  
  if(length(modelo_idx) > 0) {
    preds_test <- predecir_test(modelos_exitosos[[modelo_idx]]$modelo, 
                                modelo_nombre, 
                                as.data.frame(X_test))
    
    if(!is.null(preds_test)) {
      # Crear archivo de submission
      submission_otros <- data.frame(
        property_id = test_final$property_id,
        price = preds_test
      )
      
      # Guardar archivo
      filename <- paste0("predicciones_", tolower(gsub(" ", "_", modelo_nombre)), ".csv")
      write_csv(submission_otros, filename)
      cat("✅ Predicciones guardadas en:", filename, "\n")
    }
  }
}
...
# =============================================================================
# RECONSTRUCCIÓN RÁPIDA DE MODELOS PARA PREDICCIONES
# =============================================================================

cat("\n=== RECONSTRUYENDO MODELOS PARA PREDICCIONES ===\n")

# Preparar datos para modelos rápidos
train_rapido <- bind_cols(price = y_train, X_train)
test_rapido <- as.data.frame(X_test)

# 1. REGRESIÓN LINEAL RÁPIDA
cat("\n--- ENTRENANDO REGRESIÓN LINEAL RÁPIDA ---\n")
lm_rapido <- lm(price ~ ., data = train_rapido)
preds_lm_test <- predict(lm_rapido, newdata = test_rapido)
exportar_predicciones(preds_lm_test, "Regresion Lineal", test_ids)

# 2. ELASTIC NET RÁPIDO
cat("\n--- ENTRENANDO ELASTIC NET RÁPIDO ---\n")
library(glmnet)
X_train_matrix <- as.matrix(X_train)
X_test_matrix <- as.matrix(X_test)

en_rapido <- cv.glmnet(X_train_matrix, y_train, alpha = 0.5, nfolds = 3)
preds_en_test <- predict(en_rapido, newx = X_test_matrix, s = "lambda.min")[,1]
exportar_predicciones(preds_en_test, "Elastic Net", test_ids)

# 3. ÁRBOL DE DECISIÓN RÁPIDO
cat("\n--- ENTRENANDO ÁRBOL DECISIÓN RÁPIDO ---\n")
library(rpart)
cart_rapido <- rpart(price ~ ., data = train_rapido, method = "anova", 
                     control = rpart.control(cp = 0.01, maxdepth = 10))
preds_cart_test <- predict(cart_rapido, newdata = test_rapido)
exportar_predicciones(preds_cart_test, "Arbol Decision", test_ids)

# 4. RANDOM FOREST RÁPIDO (versión ligera)
cat("\n--- ENTRENANDO RANDOM FOREST RÁPIDO ---\n")
library(ranger)
rf_rapido <- ranger(price ~ ., data = train_rapido, 
                    num.trees = 50,  # Menos árboles para velocidad
                    mtry = floor(ncol(X_train)/3),
                    min.node.size = 5,
                    seed = 123)
preds_rf_test <- predict(rf_rapido, data = test_rapido)$predictions
exportar_predicciones(preds_rf_test, "Random Forest", test_ids)

# 5. XGBOOST RÁPIDO
cat("\n--- ENTRENANDO XGBOOST RÁPIDO ---\n")
library(xgboost)
dtrain_rapido <- xgb.DMatrix(data = as.matrix(X_train), label = y_train)
dtest_rapido <- xgb.DMatrix(data = as.matrix(X_test))

xgb_rapido <- xgb.train(
  params = list(
    objective = "reg:squarederror",
    eta = 0.1,
    max_depth = 6,
  ...
  data = dtrain_rapido,
  nrounds = 50,  # Menos iteraciones para velocidad
  verbose = 0
)

preds_xgb_test <- predict(xgb_rapido, dtest_rapido)
exportar_predicciones(preds_xgb_test, "XGBoost", test_ids)

# 6. RED NEURONAL RÁPIDA
cat("\n--- ENTRENANDO RED NEURONAL RÁPIDA ---\n")
library(nnet)
...
X_train_scaled <- as.data.frame(lapply(X_train, scale_simple))
y_train_scaled <- scale_simple(y_train)
X_test_scaled <- as.data.frame(lapply(X_test, function(x) {
  (x - mean(X_train[[which(names(X_train) == names(x))]], na.rm = TRUE)) / 
    sd(X_train[[which(names(X_train) == names(x))]], na.rm = TRUE)
}))

nn_rapido <- nnet(x = X_train_scaled, y = y_train_scaled,
                  size = 10, linout = TRUE, maxit = 100, trace = FALSE)

preds_nn_scaled <- predict(nn_rapido, X_test_scaled)
preds_nn_test <- preds_nn_scaled * sd(y_train, na.rm = TRUE) + mean(y_train, na.rm = TRUE)
exportar_predicciones(preds_nn_test, "Red Neuronal", test_ids)

# 7. SUPER LEARNER (ya lo tienes)
cat("\n--- RE-EXPORTANDO SUPER LEARNER ---\n")
exportar_predicciones(preds_test_continuas, "Super Learner", test_ids)
...
cat("\n", strrep("=", 60), "\n")
cat("📁 RESUMEN DE ARCHIVOS EXPORTADOS\n")
cat(strrep("=", 60), "\n")

archivos_exportados <- list.files(ruta_base, pattern = "predicciones_.*\\.csv$")
...
cat("1. Subir cada archivo a Kaggle Competition\n")
cat("2. Registrar el MAE/Score del leaderboard privado\n")
cat("3. Comparar con métricas de entrenamiento\n")
cat("4. Analizar overfitting (diferencia train vs test)\n")

# =============================================================================
# CALCULAR MÉTRICAS DE ENTRENAMIENTO PARA COMPARACIÓN
# =============================================================================

cat("\n", strrep("=", 60), "\n")
cat("📊 MÉTRICAS DE ENTRENAMIENTO PARA COMPARACIÓN\n")
cat(strrep("=", 60), "\n")

# Función para calcular MAE en train
calcular_mae_train <- function(modelo, nombre) {
  tryCatch({
    if(nombre == "Regresion Lineal") {
      preds <- predict(modelo, newdata = as.data.frame(X_train))
    } else if(nombre == "Elastic Net") {
      preds <- predict(modelo, newx = X_train_matrix, s = "lambda.min")[,1]
    } else if(nombre == "Arbol Decision") {
      preds <- predict(modelo, newdata = as.data.frame(X_train))
    } else if(nombre == "Random Forest") {
      preds <- predict(modelo, data = as.data.frame(X_train))$predictions
    } else if(nombre == "XGBoost") {
      preds <- predict(modelo, as.matrix(X_train))
    } else if(nombre == "Red Neuronal") {
      preds_scaled <- predict(modelo, X_train_scaled)
      preds <- preds_scaled * sd(y_train, na.rm = TRUE) + mean(y_train, na.rm = TRUE)
    }
...
mae_train <- c(
  "Super Learner" = round(mae_sl, 2),
  "Regresion Lineal" = calcular_mae_train(lm_rapido, "Regresion Lineal"),
  "Elastic Net" = calcular_mae_train(en_rapido, "Elastic Net"),
  "Arbol Decision" = calcular_mae_train(cart_rapido, "Arbol Decision"),
  "Random Forest" = calcular_mae_train(rf_rapido, "Random Forest"),
  "XGBoost" = calcular_mae_train(xgb_rapido, "XGBoost"),
  "Red Neuronal" = calcular_mae_train(nn_rapido, "Red Neuronal")
)

cat("MAE en entrenamiento por modelo:\n")
for(i in 1:length(mae_train)) {
  cat(sprintf("  %-15s: %'15.0f COP\n", names(mae_train)[i], mae_train[i]))
}

cat("\n📝 PLANTILLA PARA REGISTRO KAGGLE:\n")
cat("Modelo | MAE_Train | MAE_Kaggle | Diferencia | Overfitting_%\n")
cat("-------|-----------|------------|------------|---------------\n")
for(modelo in names(mae_train)) {
  archivo_nombre <- tolower(gsub(" ", "_", modelo))
  cat(sprintf("%-15s | %'11.0f |            |            | \n", 
              modelo, mae_train[modelo]))
}